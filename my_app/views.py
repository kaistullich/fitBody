...
from my_app.models import RegistrationForm, Login, RegistrationDB, Admin, db
...
def login():
    ...
    if form.validate_on_submit() and request.method == 'POST':
        user = RegistrationDB.query.filter_by(username=username).first()
...
def register_page():
    try:
        ...
        if request.method == "POST" and form.validate_on_submit():
            ...
            username_query = RegistrationDB.query.filter_by(username=username).first()
            # Query DB for existing email
            email_query = RegistrationDB.query.filter_by(email=email).first()
            ...
            if email_query:
                ...
                new_user = RegistrationDB(email=email, username=username, password=password)
